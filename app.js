...
app.post('/registrar_administrador', async (req, res) => {
  try {
    const existeTabla = await mongoose.connection.db.listCollections({ name: 'administradores' }).next();
    if (!existeTabla) {
...
app.post('/ingresar_Administrador', async (req, res) => {
  ...
  try {
    const administrador = await Administrador.findOne({ rut, contrasena }).exec();
...
app.post('/crear_Eleccion', async (req, res) => {
  ...
  try {
    const existeTablaCandidatos = await mongoose.connection.db.listCollections({ name: 'candidatos' }).next();
